...
def load_iterations(kin_array, iterations):
    ...
    for i in range(iterations):
        ...
        ReH_tf_itr = model(kin_array)
        ReH_tf.append(ReH_tf_itr) 
        # print("kinematics: ", kin_array)
        # print("ReH: ", ReH_tf)

    return np.array(loss, dtype=object),np.array(val_loss, dtype=object),np.array(r_square, dtype=object),np.array(val_r_square, dtype=object),np.array(ReH_tf)
...
def models_analysis(kinarray, iterations):
    ...
    loss_m, val_loss_m, r_square_m, val_r_square_m, ReH_tf_m = load_iterations(kinarray, iterations)
    loss.append(loss_m)
    val_loss.append(val_loss_m)
    r_square.append(r_square_m)
    val_r_square.append(val_r_square_m)
    ReH_tf.append(ReH_tf_m)

    ReH_tf_mean, ReH_tf_min, ReH_tf_max, ReH_tf_std = band_limits(ReH_tf_m)
    # ReH_tf_mean_m, ReH_tf_min_m, ReH_tf_max_m, ReH_tf_std_m = band_limits(ReH_tf_m)
    # ReH_tf_mean.append(ReH_tf_mean_m)
    # ReH_tf_min.append(ReH_tf_min_m)
    # ReH_tf_max.append(ReH_tf_max_m)
    # ReH_tf_std.append(ReH_tf_std_m)
    
    models_params = {'loss': loss, 'val_loss': val_loss, 'r_square': r_square, 'val_r_square': val_r_square}
    models_ReH = {'ReH_tf': ReH_tf, 'ReH_tf_mean': ReH_tf_mean, 'ReH_tf_min': ReH_tf_min, 'ReH_tf_max': ReH_tf_max, 'ReH_tf_std': ReH_tf_std}

    return models_params, models_ReH 
...
# plt.scatter(dfcols['t'], dfcols['xmi_ReH'])
# plt.errorbar(dfcols_xmi['t'], dfcols_xmi['xmi_ReH'], yerr=dfcols_xmi['exmi_ReH'], fmt='o')
# plt.show()

# Input kinematics for predictions, also contains km15 model value of ReH
...
# Get ReH for all the models listed in 'models'
models_params, models_ReH = models_analysis(kin_array, iterations)

# Make a dataframe with all the values
df_dict = {"t":kin_array}
mean_dict = {"ReH_tf_mean": models_ReH['ReH_tf_mean']}
min_dict = {"ReH_tf_min": models_ReH['ReH_tf_min']}
max_dict = {"ReH_tf_max": models_ReH['ReH_tf_max']}
std_dict = {"ReH_tf_std": models_ReH['ReH_tf_std']}
# delta_dict = {"deltaReH": dfcols['ReH_km15'] - list(np.concatenate(models_ReH['ReH_tf_mean'][c])) for c in range(N)}
...
# print('t:', df_np['t'])
# print('ReH_tf_mean:', mean_dict['ReH_tf_mean'])
# t-dep analysis plots
# plt.plot(df_np['t'], df_np['ReH_km15'], color ="red", alpha=0.5)
plt.errorbar(dfcols_xmi['t'], dfcols_xmi['xmi_ReH'], yerr=dfcols_xmi['exmi_ReH'], fmt='o', color='red', label ='$\chi$MI')
plt.plot(df_dict['t'], df_dict['ReH_tf_mean'], color ="teal", alpha=0.5, label ='DNN model')
plt.fill_between(df_dict['t'], df_np['ReH_tf_mean']-df_np['ReH_tf_std'], df_np['ReH_tf_mean']+df_np['ReH_tf_std'], color ="teal", alpha=0.3)
plt.xlabel("$t[GeV^{2}]$", fontsize = 18)
plt.ylabel("$\mathfrak{Re}\mathcal{H}$", fontsize = 18)
plt.legend(fontsize = 18)
# plt.set_title('ReH vs t xB['+ str(xB_bin)+']')
